...
public class BossShotController : MonoBehaviour
{
    ...
    public void ShotTrident(Transform target, GameObject shotPrefub, int shotNum, float shotSpeed, float delayTime)
    {
        ...
        float dx = target.position.x - gate.gameObject.transform.position.x;
        float dy = target.position.y - gate.gameObject.transform.position.y;
        //float dz = target.position.z - gate.gameObject.transform.position.z;　// 何かあったときに使うかもなので残しておく。
        ...
        // 弾の発射
        StartCoroutine(FireShot(shots, shotSpeed, new Vector2(dx, dy), delayTime));
        Invoke("CallEndAttack", attackDeray);
    }

    private void CallEndAttack()
    {
        boss.EndAttack();
    }

    // 弾の発射（弾の速度は、正規化されたベクトルをもとにshotSpeedを乗算する）
    IEnumerator FireShot(GameObject[] shots, float shotSpeed, Vector2 vec, float delay)
    {
        float perAngle = 150 / shots.Length;                     // 最大の角度を１５０度として、１弾あたりの角度を計算
        float shotAngle = (int)(shots.Length / 2) * perAngle; // targetを正面として、そこを中心に最初に弾を生成する角度を定義（下から生成する）一番上と下の真ん中の値なので１／２。
        ...
        foreach (var shot in shots)
        {
            ...
            shotAngle -= perAngle; // 次の弾に備えて設定する角度を更新
